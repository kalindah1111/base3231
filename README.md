            ...
            if current > last:
                for b in range(last + 1, current + 1):
                    ...
                    for log in logs:
                        ...
                        # We use absolute values because swap directions flip signs.
                        price_1_per_0 = None
                        if amount0 != 0:
                            price_1_per_0 = abs(amount1) / abs(amount0)

                        print(f"Block {b} | tx={tx}")
                        print(f"  {sym0}: {amount0}")
                        print(f"  {sym1}: {amount1}")

                        if price_1_per_0 is not None:
                            print(f"  Price: {price_1_per_0} {sym1} per {sym0}")
